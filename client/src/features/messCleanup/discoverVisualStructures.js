import { getSemanticType, isConnectorPath, createCanvasStructure } from './cleanupTypes.js';
...
export const _planningStateCache = new Map();
export const extractSemanticShaftEndpoints = connector => {
  ...
  return {
    ...
    arrowheadPath: parsed.hasArrowhead ? parsed.allCommands.slice(parsed.mainCommands.length) : []
  };
};
...
export const segmentIntersectsBox = (p1, p2, box) => {
  ...
  const inBox = p => p.x > box.x && p.x < box.x + box.width && p.y > box.y && p.y < box.y + box.height;
  ...
  return segmentsIntersect(p1, p2, topLeft, topRight) || segmentsIntersect(p1, p2, topRight, bottomRight) || segmentsIntersect(p1, p2, bottomRight, bottomLeft) || segmentsIntersect(p1, p2, bottomLeft, topLeft);
};
export const pointInsideBox = (p, box, margin = 0) => {
  if (!p || !box) return false;
  return p.x >= box.x - margin && p.x <= box.x + box.width + margin && p.y >= box.y - margin && p.y <= box.y + box.height + margin;
};
...
export const computeFlowCandidateGeometry = ({
  ...
  if (!isVertical) {
    const colWidths = levels.map(lvlNodes => Math.max(...lvlNodes.map(id => nodeBoundsMap.get(id).width), 100));
    ...
    const rowHeights = levels.map(lvlNodes => Math.max(...lvlNodes.map(id => nodeBoundsMap.get(id).height), 80));
...
export const evaluateCompositionQuality = ({
  ...
  const boundsList = objects.map(obj => ({
    id: obj.id,
    ...getObjectBounds(obj)
  }));
  const isVertical = orientation === 'vertical';
  ...
  if (hasMultiNodeLevels) {
    ...
    levelMap.forEach(lvlNodes => {
      if (lvlNodes.length >= 2) {
        if (!isVertical) {
          ...
          if (deltaCx <= 2) intraLevelAlignScores.push(10);else if (deltaCx <= 15) intraLevelAlignScores.push(8.5);else if (deltaCx <= 40) intraLevelAlignScores.push(6.0);else intraLevelAlignScores.push(3.5);
        } else {
          const cyVals = lvlNodes.map(n => n.cy);
          const deltaCy = Math.max(...cyVals) - Math.min(...cyVals);
          if (deltaCy <= 2) intraLevelAlignScores.push(10);else if (deltaCy <= 15) intraLevelAlignScores.push(8.5);else if (deltaCy <= 40) intraLevelAlignScores.push(6.0);else intraLevelAlignScores.push(3.5);
        }
      }
    });
    ...
    for (let i = 0; i < sortedLevels.length - 1; i++) {
      ...
      if (l1.length === 1 && l2.length >= 2) {
        ...
        if (!isVertical) {
          ...
          if (diff <= 5) centeringScores.push(10);else if (diff <= 20) centeringScores.push(8.5);else if (diff <= 50) centeringScores.push(6.0);else centeringScores.push(3.5);
        } else {
    ...
    sortedLevels.forEach(lvl => {
      ...
      if (nodes.length >= 2) {
        const sortedNodes = !isVertical ? [...nodes].sort((a, b) => a.y - b.y) : [...nodes].sort((a, b) => a.x - b.x);
    ...
    if (interLevelGaps.length > 0) {
      ...
      if (minGap < 0) interSpacingScore = 2.0;else if (maxGap > 250) interSpacingScore = 3.0;else if (maxGap > 180) interSpacingScore = 4.5;else if (maxGap > 120) interSpacingScore = 6.5;else if (maxGap > 90) interSpacingScore = 8.5;else interSpacingScore = 10;
    }
    let siblingSpacingScore = 10;
    if (siblingGaps.length > 0) {
      const maxGap = Math.max(...siblingGaps);
      const minGap = Math.min(...siblingGaps);
      if (minGap < 0) siblingSpacingScore = 2.0;else if (maxGap > 200) siblingSpacingScore = 2.5;else if (maxGap > 140) siblingSpacingScore = 4.5;else if (maxGap > 80) siblingSpacingScore = 6.5;else if (maxGap > 60) siblingSpacingScore = 8.5;else siblingSpacingScore = 10;
    }
    ...
    if (boundsList.length >= 2) {
      if (orientation === 'horizontal') {
        ...
        const deltaCy = Math.max(...cyVals) - Math.min(...cyVals);
        if (deltaCy <= 2) alignment = 10;else if (deltaCy <= 15) alignment = 9.0;else if (deltaCy <= 40) alignment = 6.5;else if (deltaCy <= 80) alignment = 4.0;else alignment = Math.max(1, 4.0 - (deltaCy - 80) / 40);
      } else {
        const cxVals = boundsList.map(b => b.cx);
        const deltaCx = Math.max(...cxVals) - Math.min(...cxVals);
        if (deltaCx <= 2) alignment = 10;else if (deltaCx <= 15) alignment = 9.0;else if (deltaCx <= 40) alignment = 6.5;else if (deltaCx <= 80) alignment = 4.0;else alignment = Math.max(1, 4.0 - (deltaCx - 80) / 40);
      }
    }
    if (boundsList.length >= 3) {
      const sorted = orientation === 'horizontal' ? [...boundsList].sort((a, b) => a.x - b.x) : [...boundsList].sort((a, b) => a.y - b.y);
      const gaps = [];
      for (let i = 0; i < sorted.length - 1; i++) {
        const gap = orientation === 'horizontal' ? sorted[i + 1].x - (sorted[i].x + sorted[i].width) : sorted[i + 1].y - (sorted[i].y + sorted[i].height);
        gaps.push(gap);
      }
  ...
  if (structureType === STRUCTURE_TYPES.FLOW && explicitEdges.length > 0) {
    ...
    if (totalChecked > 0) {
      ...
      if (ratio === 0) directionalClarity = 10;else if (ratio <= 0.3) directionalClarity = 5.5;else directionalClarity = 2.0;
    }
  }
  ...
  if (connectorAttachmentData && connectorAttachmentData.length > 0) {
    ...
    connectorAttachmentData.forEach(cd => {
      ...
      const maxError = Math.max(Number.isFinite(srcError) ? srcError : 100, Number.isFinite(tgtError) ? tgtError : 100);
      let score;
      if (maxError <= 5) score = 10;else if (maxError <= 15) score = 8.5;else if (maxError <= 30) score = 6.0;else if (maxError <= 50) score = 3.5;else score = 1.5;
  ...
  const readability = Number((alignment * 0.22 + spacing * 0.22 + directionalClarity * 0.18 + connectorCrossings * 0.08 + effectiveAttachment * 0.12 + whitespace * 0.13 + hierarchy * 0.05).toFixed(2));
  const minCore = Math.min(alignment, spacing, directionalClarity, whitespace, effectiveAttachment);
  const weighted = alignment * 0.20 + spacing * 0.20 + directionalClarity * 0.16 + connectorCrossings * 0.07 + effectiveAttachment * 0.15 + whitespace * 0.11 + hierarchy * 0.03 + readability * 0.08;
...
export const evaluateTextSafety = (rawObjects, compObjects = [], ownership = null) => {
  ...
  const nodeBoundsList = compObjects.map(o => ({
    id: o.id,
    ...getObjectBounds(o)
  }));
  ...
  rawObjects.forEach(obj => {
    ...
    const isInsideNode = bestNode ? textBounds.x >= bestNode.x - 5 && textBounds.x + textBounds.width <= bestNode.x + bestNode.width + 5 && textBounds.y >= bestNode.y - 5 && textBounds.y + textBounds.height <= bestNode.y + bestNode.height + 5 : false;
...
export const discoverVisualStructures = (workspaceModel, semanticScene = null, options = {}) => {
  ...
  if (creativeObjects.length > 0) {
    ...
    const _structPayload = {
      ...
      candidateCompositions: [{
        template: TEMPLATE_TYPES.PRESERVE,
        quality: 10
      }],
    ...
    const _struct = createCanvasStructure(_structPayload);
    structures.push(_struct);
    _planningStateCache.set(_struct.id, _structPayload);
  }
  ...
  structuralObjects.forEach(div => {
    ...
    const _struct2 = createCanvasStructure(_structPayload2);
    structures.push(_struct2);
    _planningStateCache.set(_struct2.id, _structPayload2);
  });
  ...
  sortedGraphNodes.forEach(startNode => {
    ...
    if (component.length >= 2) {
      ...
      const compEdges = explicitEdges.filter(e => component.includes(e.srcId) && component.includes(e.tgtId));
      let totalDx = 0;
      let totalDy = 0;
      compEdges.forEach(e => {
      ...
      const canonicalRelationships = compEdges.map(e => ({
        id: "rel_" + e.connId,
        type: "connectedTo",
        sourceObjectId: e.srcId,
        targetObjectId: e.tgtId,
        connectorId: e.connId,
        confidence: e.confidence,
        evidence: ["connector-topology"]
      }));
      const compObjects = component.map(id => objectMap.get(id)).filter(Boolean);
      const currentNodes = compObjects.map(obj => ({
        id: obj.id,
        ...getObjectBounds(obj)
      }));
      const currentConnectors = compEdges.map(e => {
        ...
        return {
          ...
          startPoint: shaftEndpoints ? shaftEndpoints.sourceAnchor : {
            x: srcB.cx,
            y: srcB.cy
          },
          endPoint: shaftEndpoints ? shaftEndpoints.targetAnchor : {
            x: tgtB.cx,
            y: tgtB.cy
          },
          shaftEndpoints
      ...
      const connectorAttachmentData = currentConnectors.filter(cc => cc.shaftEndpoints !== null).map(cc => ({
        connId: cc.connId,
      ...
      const nonMemberObjects = rawObjects.filter(o => !memberNodeIds.has(o.id) && !memberOwnedTextIds.has(o.id) && !memberConnIds.has(o.id));
      ...
      component.forEach(nId => {
        ...
        textIds.forEach(tId => {
          ...
          if (tObj) currentLabels.push({
            id: tId,
            parentNodeId: nId,
            ...getObjectBounds(tObj)
          });
        });
      });
      ...
      currentNodes.forEach(n => {
        ...
        curMaxX = Math.max(curMaxX, n.x + n.width);
        curMaxY = Math.max(curMaxY, n.y + n.height);
      });
      currentLabels.forEach(l => {
        curMinX = Math.min(curMinX, l.x);
        curMinY = Math.min(curMinY, l.y);
        curMaxX = Math.max(curMaxX, l.x + l.width);
        curMaxY = Math.max(curMaxY, l.y + l.height);
      });
      currentConnectors.forEach(c => {
        if (c.startPoint) {
          curMinX = Math.min(curMinX, c.startPoint.x);
          curMinY = Math.min(curMinY, c.startPoint.y);
          curMaxX = Math.max(curMaxX, c.startPoint.x);
          curMaxY = Math.max(curMaxY, c.startPoint.y);
        }
        if (c.endPoint) {
          curMinX = Math.min(curMinX, c.endPoint.x);
          curMinY = Math.min(curMinY, c.endPoint.y);
          curMaxX = Math.max(curMaxX, c.endPoint.x);
          curMaxY = Math.max(curMaxY, c.endPoint.y);
        }
      });
      ...
      const _structPayload3 = {
        ...
        movementCost,
        risk,
        reason: benefit >= 1.5 ? `Flowchart structure with ${nodeCount} connected nodes has low visual quality (${currentQualityMetrics.quality}/10); reorganizing into structured ${orientation} flow improves readability.` : `Flowchart structure with ${nodeCount} connected nodes already has clean flow composition (${currentQualityMetrics.quality}/10); preserved.`,
        relationships: canonicalRelationships
      };
      const _struct3 = createCanvasStructure(_structPayload3);
      structures.push(_struct3);
      _planningStateCache.set(_struct3.id, _structPayload3);
    }
  });
  const unverifiedConnectors = unknownConnectors.map(cId => objectMap.get(cId)).filter(Boolean).filter(o => !claimedConnectorIds.has(o.id));
  unverifiedConnectors.forEach(conn => {
    const detachedRes = findDetachedFlowAssociation(conn, candidateContainers, {
      visualStructures: structures
    });
    ...
    const otherObjects = rawObjects.filter(o => o.id !== conn.id && o.id !== srcId && o.id !== tgtId && !memberOwnedTextIds.has(o.id));
    let candA = null;
    if (repairPayload && repairPayload.repairAccepted) {
      ...
      const candAGeoComparison = {
        ...
        connectorDisplacements: {
          [conn.id]: repairPayload.pathChanged ? 20 : 0
        },
    ...
    const _struct4 = createCanvasStructure(_structPayload4);
    structures.push(_struct4);
    _planningStateCache.set(_struct4.id, _structPayload4);
  });
  unknownConnectors.filter(cId => !claimedConnectorIds.has(cId)).forEach(cId => {
    claimedObjectIds.add(cId);
    claimedConnectorIds.add(cId);
    const _structPayload5 = {
    ...
    const _struct5 = createCanvasStructure(_structPayload5);
    structures.push(_struct5);
    _planningStateCache.set(_struct5.id, _structPayload5);
  });
  ...
  if (unclaimedNotes.length >= 2) {
    ...
    noteClusters.forEach(clusterIds => {
      ...
      const _structPayload6 = {
        ...
        movementCost,
        risk,
        reason: benefit >= 1.5 ? `Cluster of ${clusterIds.length} sticky notes has disorganized layout (${currentQuality.quality}/10); compact grid arrangement restores scannability.` : `Cluster of ${clusterIds.length} sticky notes is already well-placed (${currentQuality.quality}/10); preserved.`,
        relationships: []
      };
      const _struct6 = createCanvasStructure(_structPayload6);
      structures.push(_struct6);
      _planningStateCache.set(_struct6.id, _structPayload6);
    });
  }
  ...
  conceptGroups.forEach(g => {
    ...
    if (unclaimedInGroup.length >= 2) {
      ...
      const _structPayload7 = {
        ...
        movementCost,
        risk,
        reason: benefit >= 1.5 ? `Sequence of ${unclaimedInGroup.length} objects has inconsistent alignment or spacing (${currentQuality.quality}/10); aligning establishes clean visual rhythm.` : `Sequence of ${unclaimedInGroup.length} objects is already clean (${currentQuality.quality}/10); preserved.`,
        relationships: []
      };
      const _struct7 = createCanvasStructure(_structPayload7);
      structures.push(_struct7);
      _planningStateCache.set(_struct7.id, _structPayload7);
    }
  });
    ...
    if (!claimedObjectIds.has(ann.objectId)) {
      ...
      if (parentId) {
        ...
        const _structPayload8 = {
          ...
          relationships: [{
            id: "rel_ann_" + ann.objectId,
            type: "annotates",
            sourceObjectId: ann.objectId,
        ...
        const _struct8 = createCanvasStructure(_structPayload8);
        structures.push(_struct8);
        _planningStateCache.set(_struct8.id, _structPayload8);
      }
    }
  });
  rawObjects.forEach(obj => {
    ...
    const _struct9 = createCanvasStructure(_structPayload9);
    structures.push(_struct9);
    _planningStateCache.set(_struct9.id, _structPayload9);
  });
  return structures;
};
export const generateCompositionCandidates = (visualStructures, options = {}) => {
  ...
  visualStructures.forEach(struct => {
    ...
    const availableCandidates = (cached.candidateCompositions || []).slice(0, 3);
    availableCandidates.forEach((cand, idx) => {
      const benefit = cand.compositionBenefit ?? cached.compositionBenefit ?? 0;
      const candConfidence = cand.confidence ?? struct.confidence;
      const candCost = cand.movementCost ?? cached.movementCost ?? 1.0;
      const candRisk = cand.risk ?? cached.risk ?? 1.0;
      if (candConfidence >= minCandidateConfidence && benefit >= minCompositionBenefit && candRisk <= maxRisk) {
        ...
        candidates.push({
          ...
          currentQuality: cand.currentQuality ?? cached.currentComposition?.quality,
          currentGeometry: cand.currentGeometry || cached.currentGeometry,
          ...
          reason: cand.reason || cached.reason,
          ...
          connectorRepairs: cand.connectorRepairs || cached.connectorRepairs || []
        });
      }
    });
  });
